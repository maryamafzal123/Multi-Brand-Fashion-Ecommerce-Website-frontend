...
export default function Navbar() {
  ...
  const socialLinks = [
    ...
    { label: 'Facebook', href: 'https://www.facebook.com/share/1CUc5hKLg9/', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> },
    ...
    <nav style={{ position: 'fixed', top: 0, width: '100%', zIndex: 100, background: '#0a0a0a', borderBottom: '0.5px solid rgba(184,150,12,0.2)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 4vw', height: `${NAV_H}px`, position: 'relative' }}>
        ...
        <div className="nb-left" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flex: 1, minWidth: '120px' }}>
          ...
          <div className="nb-navlinks" style={{ gap: '1.8rem' }}>
            {[{ label: 'Home', href: '/' }, { label: 'Shop Brands', href: '/products' }, { label: 'About', href: '/#about' }, { label: 'Contact', href: '/contact' }].map(l => (
              <Link key={l.href} href={l.href}
                style={{ fontSize: '0.7rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)', textDecoration: 'none', whiteSpace: 'nowrap' }}
      ...
      <Link href="/" className="nb-logo" style={{ textDecoration: 'none', textAlign: 'center', flexShrink: 0, position: 'absolute', left: '50%', transform: 'translateX(-50%)' }}>
        <Image src="/logo.png" alt="Brand Bazar by Mirsa" width={130} height={55} style={{ objectFit: 'contain',marginTop: '11px', }} />
      </Link>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {[{ label: 'Home', href: '/' }, { label: "Women's Collection", href: '/products' }, { label: 'Top Brands', href: '/products' }, { label: 'About Us', href: '/#about' }, { label: 'Contact', href: '/contact' }].map(l => (
            <Link key={l.label} href={l.href} onClick={() => setMenuOpen(false)}
              style={{ fontSize: '0.72rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.75)', textDecoration: 'none', padding: '1rem 0', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#b8960c')}
